...
class OfflineMetaRLAlgorithm(metaclass=abc.ABCMeta):
	def __init__(
		...
		print("wandb_project_name: ", self.wandb_project_name)
		print("wandb_run_name: ", self.wandb_run_name)
		print("csv_name: ", self.csv_name)
	...
	def init_buffer(self):
		...
		# self.sample is always True in any configs setting.
		if self.sample:
			for n in range(self.n_trj):
				# self.train_epoch will not be None.
				# False
				...
				# self.eval_epoch will not be None.
				# False
		...
		# 区分training task 和 testing task 对应的 trajectory path
		# training task trajectory path
		...
		print("len_sum_reward_train_lst: ", len(sum_reward_train_lst))
		print("avg_reward_train_trajectory: ", sum(sum_reward_train_lst)/len(sum_reward_train_lst))
		...
		print("len_sum_reward_eval_lst: ", len(sum_reward_eval_lst))
		print("avg_reward_eval_trajectory: ", sum(sum_reward_eval_lst) / len(sum_reward_eval_lst))
	...
	def train(self):
		...
		with open(csv_file_name, "w", encoding='utf-8', newline='') as f:
			...
			print("Creating CSV file for rewards.")
			writer = csv.writer(f)
			print("Writing csv_head.")
		...
		with open(csv_file_name, "w", encoding='utf-8', newline='') as f:
			...
			for it_ in gt.timed_for(range(self.num_iterations), save_itrs=True):
				...
				if self.allow_eval:
					...
					csv_reward_tuple = (train_returns, avg_train_return, avg_test_return)
					writer.writerow(csv_reward_tuple)
