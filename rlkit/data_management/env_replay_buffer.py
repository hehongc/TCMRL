...
class MultiTaskReplayBuffer(object):
    ...
    def random_batch_for_lap(self, task, batch_size, sequence=False):
        if sequence:
            batch, next_batch = self.task_buffers[task].random_sequence_lap(batch_size)
        else:
            batch, next_batch = self.task_buffers[task].random_batch_lap(batch_size)
