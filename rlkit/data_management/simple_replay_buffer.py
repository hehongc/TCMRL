...
class SimpleReplayBuffer(ReplayBuffer):
    ...
    def random_batch_lap(self, batch_size):
        assert self._size > 0

        indices = np.random.randint(0, self._size, batch_size)
        next_indices = indices + 1
        current_sample_data = self.sample_data(indices)
        next_sample_data = self.sample_data(next_indices)

        return current_sample_data, next_sample_data

    def random_sequence_lap(self, batch_size):
        ''' batch of trajectories '''
        # take random trajectories until we have enough
        i = 0
        indices = []
